...
class DummyActivity : AppCompatActivity() {
        ...
        formulaEditTextWatcher.setOnEditorActionListener({ v, actionId, event ->
            if (actionId == EditorInfo.IME_ACTION_DONE) {
                ...
                if (text[0] == '=') {
                    ...
                    val values = cppApi.returnUpdatedValues(text, currentCell!!.rowNumber, getCurrentColumn (currentCell!!.columnNumber), currentCell!!.columnNumber + currentCell!!.rowNumber.toString(), 1)
                    updateValues(values)
                }
                else {
    ...
    private fun closeEditText() {
        ...
        if (text[0] == '=') {
            ...
            cppApi.evaluateExpression (text, parsedTokens, currentCell!!.rowNumber, getCurrentColumn (currentCell!!.columnNumber), currentCell!!.columnNumber + currentCell!!.rowNumber.toString())
            values = cppApi.returnUpdatedValues(text, currentCell!!.rowNumber, getCurrentColumn (currentCell!!.columnNumber), currentCell!!.columnNumber + currentCell!!.rowNumber.toString(), 1)
            updateValues(values)
        }
        else {
            cppApi.storeCellValueConstant(formulaEditTextWatcher.text.toString(), currentCell!!.rowNumber, getCurrentColumn(currentCell!!.columnNumber))
